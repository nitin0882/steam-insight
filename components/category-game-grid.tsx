...
import { useGamesByCategory } from "@/hooks/use-steam-games"
...
export function CategoryGameGrid({ category }: CategoryGameGridProps) {
    const { games, isLoading, error, refetch } = useGamesByCategory(category, 18)
                    ...
                    {isLoading
                        ...
                        games.filter(game => game.id && game.id !== 0).map((game) => (
                            <Link key={game.id} href={`/game/${game.id}`} className="block">
                                <Card
                                    className="group overflow-hidden border-border/50 bg-card/50 backdrop-blur-sm hover:bg-card/80 transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-primary/10 cursor-pointer"
                                >
